...
export const create = (req, res) => {
  const staffStrength = req.body.staffStrength.map((data) => data);

  StaffStrengthRange.model.find(
    {
      staffStrength: {
        $in: req.body.staffStrength.map((data) => data.staffStrength),
      },
    },
    (err, existingData) => {
...
export const remove = (req, res) => {
  StaffStrengthRange.model.findById(req.params.id).exec((err, item) => {
    if (item) {
      item.remove(() =>
        res.sendSuccess(null, 204, 'Staff Strength has been deleted.'),
      );
    } else {
